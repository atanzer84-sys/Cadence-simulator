...
def test_star_catalog_add_and_get_star(make_star):
    catalog = StarCatalog()
    star = make_star()
...
def test_star_catalog_get_missing_star_raises():
    catalog = StarCatalog()
...
def test_star_catalog_set_and_get_offset():
    catalog = StarCatalog()
...
def test_offset_accepts_string_numbers():
    catalog = StarCatalog()
    catalog.set_offset_arcsec("s1", "1", "2")
    assert catalog.get_offset_arcsec("s1") == (1.0, 2.0)
...
def test_offset_rejects_non_numeric_strings():
    catalog = StarCatalog()
...
def test_star_catalog_get_missing_offset_raises():
    catalog = StarCatalog()
    with pytest.raises(KeyError):
...
def test_counts_reference_semantics():
    catalog = StarCatalog()
    arr = np.array([1, 2, 3])
    catalog.counts_by_id_and_band[("s1", "V")] = arr
    arr[0] = 99
