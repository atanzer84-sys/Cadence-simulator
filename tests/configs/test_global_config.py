...
def test_optional_bool_and_int_fields_omitted_use_defaults(tmp_path):
    """When write_calibration_frame_png, write_science_frames_png, produce_flux_convolution_plots and int frame fields are omitted, they use default False/0."""
    content = _base_cfg()
    for key in (
        "write_calibration_frame_png",
        "write_science_frames_png",
        "write_background_star_footprint_on_science_frame",
    ...
    assert cfg.write_calibration_frame_png is False
    assert cfg.write_science_frames_png is False
    assert cfg.write_background_star_footprint_on_science_frame is False
...
def test_optional_bool_and_int_fields_set_parsed_correctly():
    """When write_calibration_frame_png, write_science_frames_png, produce_flux_convolution_plots and int frame fields are set, they are parsed correctly."""
    cfg = gc.load_global_config(_full_cfg_path())

    assert cfg.produce_flux_convolution_plots is True
    assert cfg.write_calibration_frame_png is True
