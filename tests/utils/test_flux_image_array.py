...
def test_plot_flux_and_photons_windows_respects_enabled_channels(monkeypatch, tmp_path, make_star, make_global_config):
    """plot_flux_and_photons_windows only writes PNGs for enabled channels."""
    cfg = make_global_config(run_nuv=True, run_vis=False, run_nir=True)
    monkeypatch.setattr("utils.flux_image_array.get_global_config", lambda: cfg)
...
def test_plot_flux_and_photons_windows_full_writes_full_range_png(monkeypatch, tmp_path, make_star, make_global_config):
