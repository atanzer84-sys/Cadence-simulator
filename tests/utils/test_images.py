...
from utils.images import write_calibration_frame_png, write_science_frames_png


def _channel_for_image():
    # Minimal SpectroscopyChannel-like object for write_calibration_frame_png
...
def test_write_calibration_frame_png_creates_file_with_expected_name(tmp_path):
    ctx = _ctx_for_image(tmp_path)
    ch = _channel_for_image()

    array = np.ones((ch.y_pixels, ch.x_pixels), dtype=float)

    write_calibration_frame_png(array, "BIAS", ctx, ch, show_stats=True)
...
def test_write_calibration_frame_png_without_stats(tmp_path):
    ctx = _ctx_for_image(tmp_path)
    ch = _channel_for_image()

    array = np.ones((ch.y_pixels, ch.x_pixels), dtype=float)

    write_calibration_frame_png(array, "DARK", ctx, ch, show_stats=False)
...
def _star_for_frames():
    # Minimal Star-like object for write_science_frames_png
    return SimpleNamespace(
        name="HD 2685",
        mass=1.1,
        distance_pc=100.0,
...
def test_write_science_frames_png_creates_numbered_files(tmp_path):
    ...
    write_science_frames_png(frames, headers, frame_type="BIAS", channel_tag="NUV", ctx=ctx, star=star, show_stats=True)
...
def test_write_science_frames_png_no_frames_does_nothing(tmp_path):
    ctx = _ctx_for_frames(tmp_path)
    star = _star_for_frames()

    write_science_frames_png([], [], frame_type="BIAS", channel_tag="NUV", ctx=ctx, star=star, show_stats=False)
...
from utils.images import format_header, write_science_frames_png
...
def test_write_science_frames_png_empty_frames_returns_without_creating_files(tmp_path):
    """write_science_frames_png with empty frames returns early; no PNG files created."""
    ctx = SimpleNamespace(output_dir=tmp_path)
    write_science_frames_png([], [], "BIAS", "NUV", ctx, _dummy_star())
    assert list(tmp_path.glob("*.png")) == []


def test_write_science_frames_png_writes_one_file(tmp_path):
    """write_science_frames_png writes one PNG file per frame."""
    ctx = SimpleNamespace(output_dir=tmp_path)
    frame = np.zeros((4, 4), dtype=float)
    hdr = {}

    write_science_frames_png([frame], [hdr], "bias", "NUV", ctx, _dummy_star())
