...
def test_plot_star_counts_vs_noise_spectroscopy_writes_png(tmp_path, make_run_context, make_spectroscopy_channel):
    ...
    ch = make_spectroscopy_channel(channel_name="VIS", exposure_s=60.0)
...
def test_plot_star_counts_vs_noise_spectroscopy_with_star_writes_png(tmp_path, make_star, make_run_context, make_spectroscopy_channel):
    """plot_star_counts_vs_noise_spectroscopy accepts star and still writes PNG."""
    wavelength = np.linspace(4000.0, 8000.0, 50, dtype=float)
    counts_s_px = np.ones(50, dtype=float) * 0.1
    ch = make_spectroscopy_channel(channel_name="NUV")
    ctx = make_run_context(output_dir=tmp_path)
...
def test_plot_star_counts_vs_noise_photometry_writes_png(tmp_path, make_run_context, make_photometry_channel):
    """plot_star_counts_vs_noise_photometry writes a PNG to ctx.output_dir."""
    rate_image_e_s = np.ones((10, 10), dtype=np.float32) * 0.5
    ch = make_photometry_channel(channel_name="NIR", exposure_s=30.0)
    ctx = make_run_context(output_dir=tmp_path)
...
def test_plot_star_counts_vs_noise_photometry_with_star_writes_png(tmp_path, make_star, make_run_context, make_photometry_channel):
    """plot_star_counts_vs_noise_photometry accepts star and still writes PNG."""
    rate_image_e_s = np.ones((8, 8), dtype=np.float32) * 1.0
    ch = make_photometry_channel(channel_name="NIR")
    ctx = make_run_context(output_dir=tmp_path)
