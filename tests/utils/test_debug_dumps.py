...
def test_write_frames_fits_no_frames_returns_without_creating_files(tmp_path):
    write_frames_fits([], [], "BIAS", "NUV", tmp_path)
    assert list(tmp_path.glob("*.fits")) == []


def test_write_frames_fits_writes_one_file_with_header(tmp_path):
    frame = np.zeros((2, 3), dtype=float)
    hdr = fits.Header()
    hdr["FILETYPE"] = "BIAS"

    write_frames_fits([frame], [hdr], "BIAS", "NUV", tmp_path)
...
def test_write_frames_fits_zips_frames_and_headers_stops_at_shorter(tmp_path):
    ...
    write_frames_fits([frame0, frame1], [hdr0], "DARK", "VIS", tmp_path)
