"""Tests for load_excel: load_matching_excel_row_from_excel, map_to_planet_or_star_dictionary, _strip_hash_from_excel_column."""

from pathlib import Path

import pytest
from openpyxl import Workbook

from loaders.load_excel import (
    load_matching_excel_row_from_excel,
    load_excel_cfg,
    _strip_hash_from_excel_column,
    _normalize_excel_value,
)


def _write_excel(path: Path, headers: list, rows: list[list]) -> None:
    wb = Workbook()
    ws = wb.active
    ws.append(headers)
    for row in rows:
        ws.append(row)
    wb.save(path)

# Tests: map_to_planet_or_star_dictionary
# Behavior: emits warning for unmapped headers and preserves mapped values
def test_map_to_planet_or_star_dictionary_logs_unknown_headers(tmp_path, caplog):
    from loaders.load_excel import map_to_planet_or_star_dictionary

    # Excel row contains an unmapped header "weird_col"
    row = {
        "pl_name": "Planet X b",
        "st_teff": 5000,
        "weird_col": 123,   # unmapped
    }

    mapping = {
        "planet": {"orbital_period": "pl_orbper"},
        "star": {"effective_temperature": "st_teff"},
        "required_planetary_parameters": [],
        "required_stellar_parameters": [],
    }

    with caplog.at_level("WARNING"):
        planet_params, star_params = map_to_planet_or_star_dictionary(row, mapping, "Planet X")

    assert any(
        rec.levelname == "WARNING" and "unmapped" in rec.message.casefold()
        for rec in caplog.records
    )
    assert planet_params == {}
    assert star_params["effective_temperature"] == 5000
    assert star_params["name"] == "Planet X"


# Tests: map_to_planet_or_star_dictionary
# Behavior: missing required canonical keys remain absent
def test_map_to_planet_or_star_dictionary_detects_missing_required_keys():
    from loaders.load_excel import map_to_planet_or_star_dictionary

    row = {
        "pl_name": "Planet X b",
        "st_teff": 5000,
    }

    mapping = {
        "planet": {"orbital_period": "pl_orbper"},  # missing in row
        "star": {"effective_temperature": "st_teff"},
        "required_planetary_parameters": ["orbital_period"],
        "required_stellar_parameters": ["effective_temperature", "radius"],
    }

    planet_params, star_params = map_to_planet_or_star_dictionary(row, mapping, "Star X")

    # orbital_period missing
    assert "orbital_period" not in planet_params

    # radius missing
    assert "radius" not in star_params



# Tests: map_to_planet_or_star_dictionary
# Behavior: injects target_name into star params
def test_map_to_planet_or_star_dictionary_inserts_star_name():
    from loaders.load_excel import map_to_planet_or_star_dictionary

    row = {
        "pl_name": "Planet X b",
        "st_teff": 5000,
    }

    mapping = {
        "planet": {},
        "star": {"effective_temperature": "st_teff"},
        "required_planetary_parameters": [],
        "required_stellar_parameters": [],
    }

    _, star_params = map_to_planet_or_star_dictionary(row, mapping, "Star X")

    assert star_params["name"] == "Star X"
    assert star_params["effective_temperature"] == 5000


# Tests: map_to_planet_or_star_dictionary
# Behavior: empty string value normalizes to None and is logged as missing
def test_map_to_planet_or_star_dictionary_empty_string_treated_as_missing(caplog):
    """Empty string '' is normalized to None and treated as missing for required stellar parameters."""
    from loaders.load_excel import map_to_planet_or_star_dictionary

    row = {
        "pl_name": "Planet X b",
        "st_teff": "",  # empty string
    }

    mapping = {
        "planet": {},
        "star": {"effective_temperature": "st_teff"},
        "required_planetary_parameters": [],
        "required_stellar_parameters": ["effective_temperature"],
    }

    with caplog.at_level("INFO"):
        _, star_params = map_to_planet_or_star_dictionary(row, mapping, "Star X")

    assert star_params["effective_temperature"] is None
    assert any(
        "effective_temperature" in rec.message and "Missing required star" in rec.message
        for rec in caplog.records
    )

# --- load_matching_excel_row_from_excel: pl_name not found ---
def test_pl_name_not_found_raises(tmp_path: Path) -> None:
    path = tmp_path / "targets.xlsx"
    _write_excel(
        path,
        headers=["pl_name", "st_teff"],
        rows=[["Other Star b", 5000]],
    )
    with pytest.raises(ValueError) as exc_info:
        load_matching_excel_row_from_excel(path, "HD 202772 A")
    assert "No target found" in str(exc_info.value)
    assert "HD 202772 A" in str(exc_info.value)


# --- load_matching_excel_row_from_excel: no pl_name column ---
def test_excel_no_pl_name_column_raises(tmp_path: Path) -> None:
    path = tmp_path / "targets.xlsx"
    _write_excel(
        path,
        headers=["planet", "st_teff"],
        rows=[["HD 202772 A b", 5000]],
    )
    with pytest.raises(ValueError) as exc_info:
        load_matching_excel_row_from_excel(path, "HD 202772 A")
    assert "no 'pl_name' column" in str(exc_info.value)


# --- load_matching_excel_row_from_excel: empty pl_name in row ---
def test_excel_empty_pl_name_in_row_raises(tmp_path: Path) -> None:
    path = tmp_path / "targets.xlsx"
    _write_excel(
        path,
        headers=["pl_name", "st_teff"],
        rows=[[None, 5000]],  # empty pl_name in first data row
    )
    with pytest.raises(ValueError) as exc_info:
        load_matching_excel_row_from_excel(path, "HD 202772 A")
    assert "No target found" in str(exc_info.value)


# Tests: load_matching_excel_row_from_excel
# Behavior: rows shorter than headers are padded with None values
def test_excel_row_shorter_than_headers_is_padded(tmp_path: Path) -> None:
    path = tmp_path / "targets.xlsx"
    _write_excel(
        path,
        headers=["pl_name", "st_teff", "pl_orbper", "extra_col"],
        rows=[["HD 202772 A b", 5000]],
    )

    row_dict, target_name = load_matching_excel_row_from_excel(path, "HD 202772 A")

    assert row_dict is not None
    assert row_dict.get("pl_name") == "HD 202772 A b"
    assert row_dict.get("st_teff") == 5000
    assert row_dict.get("pl_orbper") is None
    assert row_dict.get("extra_col") is None
    assert target_name == "HD 202772 A"


# --- load_matching_excel_row_from_excel: one found ---
def test_pl_name_one_found_returns_row_and_target_name(tmp_path: Path) -> None:
    path = tmp_path / "targets.xlsx"
    _write_excel(
        path,
        headers=["pl_name", "st_teff", "pl_orbper"],
        rows=[["HD 202772 A b", 5000, 3.4]],
    )
    row_dict, target_name = load_matching_excel_row_from_excel(path, "HD 202772 A")
    assert row_dict is not None
    assert row_dict.get("pl_name") == "HD 202772 A b"
    assert row_dict.get("st_teff") == 5000
    assert row_dict.get("pl_orbper") == 3.4
    assert target_name == "HD 202772 A"


# --- load_matching_excel_row_from_excel: two found (first match wins) ---
def test_pl_name_two_found_first_wins(tmp_path: Path) -> None:
    path = tmp_path / "targets.xlsx"
    _write_excel(
        path,
        headers=["pl_name", "st_teff"],
        rows=[
            ["HD 202772 A b", 5000],
            ["HD 202772 A c", 5100],
        ],
    )
    row_dict, _ = load_matching_excel_row_from_excel(path, "HD 202772 A")
    assert row_dict is not None
    assert row_dict.get("pl_name") == "HD 202772 A b"
    assert row_dict.get("st_teff") == 5000


# --- load_matching_excel_row_from_excel: case insensitive ---
def test_pl_name_match_case_insensitive(tmp_path: Path) -> None:
    path = tmp_path / "targets.xlsx"
    _write_excel(
        path,
        headers=["pl_name", "st_teff"],
        rows=[["HD 202772 A b", 5000]],
    )
    row_dict, _ = load_matching_excel_row_from_excel(path, "hd 202772 a")
    assert row_dict is not None
    assert row_dict.get("pl_name") == "HD 202772 A b"




# --- load_matching_excel_row_from_excel: header normalization (# prefix) ---
def test_excel_header_with_hash_normalized(tmp_path: Path) -> None:
    path = tmp_path / "targets.xlsx"
    wb = Workbook()
    ws = wb.active
    ws.append(["#pl_name", "st_teff"])  # # is stripped from header
    ws.append(["HD 202772 A b", 5000])
    wb.save(path)
    row_dict, _ = load_matching_excel_row_from_excel(path, "HD 202772 A")
    assert row_dict.get("pl_name") == "HD 202772 A b"
    assert row_dict.get("st_teff") == 5000


# --- load_excel_cfg ---
def test_load_excel_cfg_missing_file_raises(tmp_path: Path) -> None:
    missing_path = tmp_path / "excel_mapping.cfg"
    with pytest.raises(FileNotFoundError, match="Excel mapping file not found"):
        load_excel_cfg(missing_path)


def test_load_excel_cfg_missing_required_sections_returns_empty_lists(tmp_path: Path) -> None:
    """Missing [required_planetary_parameters] / [required_stellar_parameters] yields empty lists."""
    cfg_path = tmp_path / "excel_mapping.cfg"
    cfg_path.write_text(
        "\n".join(["[stars]", "effective_temperature = st_teff", ""]) + "\n",
        encoding="utf-8",
    )
    mapping = load_excel_cfg(cfg_path)
    assert mapping["required_planetary_parameters"] == []
    assert mapping["required_stellar_parameters"] == []


@pytest.mark.parametrize("keys_value", ["", "   ", ",", ", ,", "  ,  ,  "])
def test_load_excel_cfg_empty_keys_returns_empty_list(tmp_path: Path, keys_value: str) -> None:
    """Empty keys = or keys = , , yields empty list."""
    cfg_path = tmp_path / "excel_mapping.cfg"
    cfg_path.write_text(
        "\n".join(
            [
                "[stars]",
                "effective_temperature = st_teff",
                "",
                "[required_stellar_parameters]",
                f"keys = {keys_value}",
            ]
        )
        + "\n",
        encoding="utf-8",
    )
    mapping = load_excel_cfg(cfg_path)
    assert mapping["required_stellar_parameters"] == []


# --- _strip_hash_from_excel_column ---
def test_strip_hash_from_excel_column_returns_none_for_none() -> None:
    assert _strip_hash_from_excel_column(None) is None


def test_strip_hash_from_excel_column_strips_whitespace() -> None:
    assert _strip_hash_from_excel_column("  pl_name  ") == "pl_name"


def test_strip_hash_from_excel_column_removes_leading_hash() -> None:
    assert _strip_hash_from_excel_column("#pl_name") == "pl_name"

def test_pl_name_does_not_match_on_prefix_only(tmp_path: Path) -> None:
    """
    Regression: previously we used startswith(), so a short prefix like
    'TIC 3938183' could match the wrong star ('TIC 393818343 b').
    We now require exact basename match.
    """
    path = tmp_path / "targets.xlsx"
    _write_excel(
        path,
        headers=["pl_name", "st_teff"],
        rows=[
            ["TIC 393818343 b", 5000],
            ["TIC 393818399 b", 5100],
        ],
    )

    with pytest.raises(ValueError) as exc_info:
        load_matching_excel_row_from_excel(path, "TIC 3938183")

    assert "No target found" in str(exc_info.value)
    assert "TIC 3938183" in str(exc_info.value)


def test_pl_name_matches_star_basename_ignoring_planet_designator(tmp_path: Path) -> None:
    """
    Star basename should match even if Excel rows are planet rows (b/c/d...).
    """
    path = tmp_path / "targets.xlsx"
    _write_excel(
        path,
        headers=["pl_name", "st_teff", "pl_orbper"],
        rows=[
            ["TIC 393818343 b", 5000, 3.4],
            ["TIC 393818343 c", 5050, 7.8],
        ],
    )

    row_dict, target_name = load_matching_excel_row_from_excel(path, "TIC 393818343")

    assert target_name == "TIC 393818343"
    assert row_dict["pl_name"] in ("TIC 393818343 b", "TIC 393818343 c")

@pytest.mark.parametrize("pl_name,star_target,expected_pl_name", [
    ("HF 123 b", "HF 123", "HF 123 b"),
    ("KELT-19 A b", "KELT-19 A", "KELT-19 A b"),
])
def test_excel_strips_planet_designator_and_returns_star_basename(tmp_path: Path, pl_name: str, star_target: str, expected_pl_name: str) -> None:
    """Strips trailing planet letter (b/c/d...) and returns star basename, including lettered stars (e.g. KELT-19 A)."""
    path = tmp_path / "targets.xlsx"
    _write_excel(
        path,
        headers=["pl_name", "st_teff"],
        rows=[[pl_name, 5000]],
    )

    row_dict, target_name = load_matching_excel_row_from_excel(path, star_target)

    assert row_dict["pl_name"] == expected_pl_name
    assert target_name == star_target


def test_excel_internal_spaces_preserved_in_basename(tmp_path: Path) -> None:
    path = tmp_path / "targets.xlsx"
    _write_excel(
        path,
        headers=["pl_name", "st_teff"],
        rows=[["HF   123    b", 5000]],
    )

    _, target_name = load_matching_excel_row_from_excel(path, "HF   123")
    assert target_name == "HF   123"

def test_excel_uppercase_planet_letter(tmp_path: Path) -> None:
    path = tmp_path / "targets.xlsx"
    _write_excel(
        path,
        headers=["pl_name", "st_teff"],
        rows=[["HF 123 B", 5000]],
    )

    _, target_name = load_matching_excel_row_from_excel(path, "HF 123")
    assert target_name == "HF 123"

def test_excel_trailing_whitespace_in_pl_name(tmp_path: Path) -> None:
    path = tmp_path / "targets.xlsx"
    _write_excel(
        path,
        headers=["pl_name", "st_teff"],
        rows=[["HF 123 b   ", 5000]],
    )

    _, target_name = load_matching_excel_row_from_excel(path, "HF 123")
    assert target_name == "HF 123"


# --- load_matching_excel_row_from_excel: target input with surrounding whitespace ---
def test_pl_name_match_trims_target_input_whitespace(tmp_path: Path) -> None:
    path = tmp_path / "targets.xlsx"
    _write_excel(
        path,
        headers=["pl_name", "st_teff"],
        rows=[["HD 202772 A b", 5000]],
    )

    row_dict, target_name = load_matching_excel_row_from_excel(path, "  HD 202772 A   ")

    assert row_dict is not None
    assert row_dict.get("pl_name") == "HD 202772 A b"
    assert target_name == "  HD 202772 A   "

# Ensures load_excel_cfg preserves the case of canonical keys and trims required key lists correctly.
def test_load_excel_cfg_preserves_case_and_parses_required_keys(tmp_path):
    from loaders.load_excel import load_excel_cfg

    cfg_path = tmp_path / "excel_mapping.cfg"
    cfg_path.write_text(
        "\n".join(
            [
                "[stars]",
                "effective_temperature = st_teff",
                "log_R = st_logr",
                "",
                "[required_stellar_parameters]",
                "keys = effective_temperature, log_R ,  ",
            ]
        )
        + "\n",
        encoding="utf-8",
    )

    mapping = load_excel_cfg(cfg_path)

    assert mapping["star"]["effective_temperature"] == "st_teff"
    assert mapping["star"]["log_R"] == "st_logr"
    assert mapping["required_stellar_parameters"] == ["effective_temperature", "log_R"]


# Tests: _normalize_excel_value
# Behavior: converts comma-decimal strings to float
def test_normalize_excel_value_converts_comma_decimal_to_float():
    assert _normalize_excel_value("3,4") == 3.4


# Tests: _normalize_excel_value
# Behavior: trims surrounding whitespace before comma-decimal conversion
def test_normalize_excel_value_trims_and_converts_comma_decimal():
    assert _normalize_excel_value("  3,4  ") == 3.4


# Tests: _normalize_excel_value
# Behavior: keeps dot-decimal strings unchanged
def test_normalize_excel_value_keeps_dot_decimal_string():
    assert _normalize_excel_value("3.4") == "3.4"


# Tests: _normalize_excel_value
# Behavior: returns original string when comma conversion fails
def test_normalize_excel_value_keeps_non_numeric_comma_string():
    assert _normalize_excel_value("abc,def") == "abc,def"


# Tests: _normalize_excel_value
# Behavior: empty string normalizes to None
def test_normalize_excel_value_empty_string_to_none():
    assert _normalize_excel_value("") is None
