...
from loaders import load_channel
...
def test_ok_parses_pixel_scale_and_first_last_columns(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(load_channel, "get_repo_root", lambda base_dir=None: tmp_path)
    ...
    wl, ea, pixel_scale = load_channel.load_effective_area_file(fname)
...
def test_missing_file_raises_valueerror(monkeypatch, tmp_path):
    ...
    with pytest.raises(ValueError) as exc:
        load_channel.load_effective_area_file("does_not_exist.txt")
...
def test_missing_pixel_scale_header_raises_valueerror(monkeypatch, tmp_path):
    ...
    with pytest.raises(ValueError) as exc:
        load_channel.load_effective_area_file(fname)
...
def test_extra_numeric_columns_first_and_last_used(monkeypatch, tmp_path):
    ...
    wl, ea, _ = load_channel.load_effective_area_file(fname)
    assert np.allclose(wl, np.array([1000.0, 1100.0]))
...
def test_spread_empty_filename_returns_none_triplet(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(load_channel, "get_repo_root", lambda base_dir=None: tmp_path)
    pos, w, wl = load_channel.load_spread_profile_file("", "NUV")
    assert pos is None
    assert w is None
    assert wl is None
...
def test_spread_ok_parses_header_positions_and_weights(monkeypatch, tmp_path):
    ...
    pos, w, wl = load_channel.load_spread_profile_file(fname, "NUV")
...
def test_spread_missing_file_raises_valueerror(monkeypatch, tmp_path):
    ...
    with pytest.raises(ValueError) as exc:
        load_channel.load_spread_profile_file("does_not_exist.txt", "NUV")
...
def test_spread_missing_pixels_header_line_raises_valueerror(monkeypatch, tmp_path):
    ...
    with pytest.raises(ValueError) as exc:
        load_channel.load_spread_profile_file(fname, "NUV")
