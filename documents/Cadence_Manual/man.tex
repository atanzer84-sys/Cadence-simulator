...
\begin{enumerate}
    ...
    \item \textbf{Background star loading and convolution} --- \texttt{lookup\_background\_stars()} loads or queries the background star catalogue. \texttt{populate\_background\_star\_counts()} runs the flux pipeline and detector convolution for each background star and caches the results in the \texttt{StarCatalog}.

    \item \textbf{Science frame generation} --- \texttt{build\_science\_images()} assembles and writes all science frames for a channel. Each frame combines bias, dark, flat-field, target signal, photon noise, diffuse background, cosmic rays, and background star contributions. Frames are written as FITS files; PNG outputs are optional. The internal structure of a science frame is described in Section~\ref{sec:science_image}.
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.85\linewidth]{images/FluxPipeline.png}
...
\begin{itemize}
    ...
    \item \texttt{run\_photon\_flux\_density\_pipeline()} executes the photon flux density pipeline for the given \texttt{Star} object on the trimmed wavelength grid.
          \begin{itemize}
              ...
              \item \texttt{convert\_stellar\_model\_to\_flux()} converts the photospheric model from spectral flux density per unit frequency to spectral flux density per unit wavelength and scales it to the spectral flux density of the star.
              ...
              \item \texttt{apply\_ism\_absorption()} applies interstellar medium absorption at the Mg\,\textsc{ii}, Mg\,\textsc{i}, and Fe\,\textsc{ii} resonance lines if enabled in \texttt{global.cfg}.
              \item \texttt{compute\_flux\_at\_earth()} applies geometric dilution via the inverse-square law to convert spectral luminosity to spectral flux at Earth.
              \item \texttt{apply\_unred()} applies continuum dereddening using the Fitzpatrick (1999) extinction curve.
              \item \texttt{convert\_flux\_to\_photons()} converts the dereddened spectral energy flux from erg\,s$^{-1}$\,cm$^{-2}$\,\AA$^{-1}$ to photon flux density in photons\,s$^{-1}$\,cm$^{-2}$\,\AA$^{-1}$.
          \end{itemize}


    \item \texttt{load\_model\_for\_temperature()} selects the stellar atmosphere model closest to the target effective temperature and loads it from \texttt{data/models/}. Models are cached so repeated calls for the same temperature do not reload from disk.

    \item \texttt{convert\_stellar\_model\_to\_flux()} converts the model from surface flux density per unit frequency to spectral luminosity by applying the stellar surface area and the $F_\nu \rightarrow F_\lambda$ conversion.
