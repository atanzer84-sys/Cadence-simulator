...
Detector image preparation is handled per channel by \texttt{prepare\_detector\_image\_spectroscopy()} for NUV and VIS, and \texttt{prepare\_detector\_image\_photometry()} for NIR, both in \texttt{src/instrument/prepare\_detector\allowbreak\_images.py}. They compute the raw target signal on the detector in electrons\,s$^{-1}$\,pixel$^{-1}$ by convolving the photon flux with the instrument response and spreading the signal onto the 2D detector using either a wavelength-dependent cross-dispersion profile (spectroscopy) or a PSF (photometry). The detector mapping flow is illustrated in Figure~\ref{fig:detector}.
...
\begin{itemize}
    ...
    \item For spectroscopy channels (NUV, VIS), \texttt{spread\_target\_star\_spectrum\_to\_2d()} spreads the 1D count rate onto the 2D detector using either a wavelength-dependent cross-dispersion profile or a Gaussian fallback.

    \item For the NIR photometry channel, \texttt{spread\_1d\_photometry\_to\_2d()} places the total count rate onto the detector using the PSF, centred at the source position.
