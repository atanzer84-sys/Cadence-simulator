...
def generate_background_star_visibility_on_science_frame(merged_image: np.ndarray, background_star_image: np.ndarray, ctx: RunContext, channel: Channel, star: Star, index: int, background_star_bands: dict[str, dict[str, float]] | None = None, background_star_arcs: dict[str, list[tuple[int, int]]] | None = None, inverted: bool = False) -> None:
    frame_type = "SCIENCE PANEL"

    title = format_frame_title(channel.channel_name, frame_type, star)
    per_panel_stats = _prepare_background_star_panel_stats(merged_image, background_star_image, channel)
    panel_shapes = [merged_image.shape, background_star_image.shape, merged_image.shape]
    ...
    bg_arr_display, bg_vmin, bg_vmax = _compute_bg_display_range(background_star_image)
    if inverted:
        bg_arr_display = bg_arr_display.max() - bg_arr_display
        bg_vmin, bg_vmax = _compute_display_range(bg_arr_display)

    mask_dilated, overlay, has_bg = _compute_bg_mask_overlay(background_star_image, channel, background_star_bands, background_star_arcs)

    _draw_science_panel(fig, gs, merged_display, merged_vmin, merged_vmax, nx, ny)
    _draw_background_panel(fig, gs, bg_arr_display, has_bg, bg_vmin, bg_vmax, nx, ny)
    _draw_overlay_panel(fig, gs, merged_display, merged_vmin, merged_vmax, channel, background_star_bands, background_star_arcs, mask_dilated, overlay)
...
def _prepare_background_star_panel_stats(merged_image: np.ndarray, background_star_image: np.ndarray, channel: Channel) -> list[tuple[dict | None, list[str]]]:
    ...
    per_panel_stats[1] = _build_bg_star_panel_stats_row(background_star_image, channel)
    per_panel_stats[2] = build_stats_row(merged_image, channel, "science")
    return per_panel_stats

def _build_bg_star_panel_stats_row(background_star_image: np.ndarray, channel: Channel) -> tuple[dict, list[str]]:
    bg_keys = STATS_KEYS["BG_STARS"]
    has_bg = np.any(background_star_image > 0.0)
    if has_bg:
        values = {
            "MEAN": float(np.mean(background_star_image)),
            "MEDIAN": float(np.median(background_star_image)),
            "STDDEV": float(np.std(background_star_image, ddof=0)),
            "MIN": float(np.min(background_star_image)),
            "MAX": float(np.max(background_star_image)),
...
def _compute_bg_display_range(background_star_image: np.ndarray) -> tuple[np.ndarray, float, float]:
    bg_arr_display = np.arcsinh(background_star_image)
    bg_positive = bg_arr_display[background_star_image > 0.0]
...
def _compute_bg_mask_overlay(background_star_image: np.ndarray, channel: Channel, background_star_bands: dict[str, dict[str, float]] | None, background_star_arcs: dict[str, list[tuple[int, int]]] | None) -> tuple[np.ndarray, np.ndarray, bool]:
    mask = background_star_image > 0.0
...
def _draw_science_panel(fig: Any, gs: Any, merged_display: np.ndarray, merged_vmin: float, merged_vmax: float, nx: int, ny: int) -> None:
...
def _draw_background_panel(fig: Any, gs: Any, bg_arr_display: np.ndarray, has_bg: bool, bg_vmin: float, bg_vmax: float, nx: int, ny: int) -> None:
    ...
    _style_detector_panel_axis(ax2, nx, ny, "Background Stars")

def _draw_overlay_panel(fig: Any, gs: Any, merged_display: np.ndarray, merged_vmin: float, merged_vmax: float, channel: Channel, background_star_bands: dict[str, dict[str, float]] | None, background_star_arcs: dict[str, list[tuple[int, int]]] | None, mask_dilated: np.ndarray, overlay: np.ndarray) -> None:
    ax3 = _add_overlay_axis(fig, gs, merged_display, merged_vmin, merged_vmax)
    _draw_overlay_by_channel(ax3, channel, background_star_bands, background_star_arcs, mask_dilated, overlay, merged_display.shape[0])
    ny, nx = merged_display.shape
...
def _add_overlay_axis(fig: Any, gs: Any, merged_image: np.ndarray, merged_vmin: float, merged_vmax: float) -> Any:
    ax3 = fig.add_subplot(gs[8, 0])
    ax3.imshow(merged_image, origin="lower", aspect="equal", cmap="gray", vmin=merged_vmin, vmax=merged_vmax)
    return ax3

def _draw_overlay_band_mode(ax3: Any, background_star_bands: dict[str, dict[str, float]] | None, ny: int) -> None:
...
def _draw_overlay_arc_mode(ax3: Any, mask_dilated: np.ndarray, overlay: np.ndarray) -> None:
    _draw_mask_footprint(ax3, mask_dilated, overlay)
    legend_lines = [Line2D([0], [0], color="#00FF66", lw=1.2, label="Background Star Arc (90% Flux)")]
    ax3.legend(handles=legend_lines, loc="upper right", fontsize=8, frameon=True)

def _draw_overlay_by_channel(ax3: Any, channel: Channel, background_star_bands: dict[str, dict[str, float]] | None, background_star_arcs: dict[str, list[tuple[int, int]]] | None, mask_dilated: np.ndarray, overlay: np.ndarray, ny: int) -> None:
    if isinstance(channel, PhotometryChannel):
        if background_star_arcs is not None and len(background_star_arcs) > 0:
            _draw_overlay_arc_mode(ax3, mask_dilated, overlay)
        else:
            _draw_mask_footprint(ax3, mask_dilated, overlay)
    else:
        if background_star_bands is not None and len(background_star_bands) > 0:
            _draw_overlay_band_mode(ax3, background_star_bands, ny)
        else:
            _draw_mask_footprint(ax3, mask_dilated, overlay)

def _draw_mask_footprint(ax: Any, mask_dilated: np.ndarray, overlay: np.ndarray) -> None:
    if np.any(mask_dilated):
        ax.imshow(overlay, origin="lower", aspect="equal", cmap="Reds", alpha=0.2, interpolation="nearest")
        ax.contour(mask_dilated.astype(float), levels=[0.5], colors="#00FF66", linewidths=0.6, alpha=0.95)
