...
def _build_science_image_without_bg_stars(spectra_component, background_component, channel: Channel, ctx: RunContext, cfg: GlobalConfig, star: Star, frame_index: int):
    ...
    if frame_index < 1:
        ctx.write_calibration_frame_png(spectra_component, "SCIENCE_SIGNAL_ONLY", ctx, channel, star=star, index=frame_index)
...
def build_science_image_photometry(nir_rate, channel: PhotometryChannel, ctx: RunContext, cfg: GlobalConfig, star: Star, background_stars_catalog: StarCatalog):
    ...
    bias = generate_bias_image(channel)
    image += bias
    ctx.write_calibration_frame_png(image, "SCIENCE_BIAS", ctx, channel, star=star)

    dark = generate_dark_image(channel)
    image += dark
    ctx.write_calibration_frame_png(image, "SCIENCE_DARK", ctx, channel, star=star)

    spectra = nir_rate * exposure
    ctx.write_calibration_frame_png(spectra, "SIGNAL_ONLY", ctx, channel, star=star)
    image += spectra
    ctx.write_calibration_frame_png(image, "SCIENCE_SPECTRA", ctx, channel, star=star)

    photon_noise = apply_photon_noise_gauss_from_spectra2d(nir_rate*exposure, channel, ctx, star)
    image += photon_noise
    ctx.write_calibration_frame_png(image, "SCIENCE_PHOTON_NOISE", ctx, channel, star=star)

    background = generate_background_image(channel, ctx, star)
    image += background
    ctx.write_calibration_frame_png(image, "SCIENCE_BACKGROUND", ctx, channel)
    
    # TODO BACKGROUND STAR GENERATION

    cosmic = generate_cosmic_rays(ctx, channel, cfg, star)
    image += cosmic
    ctx.write_calibration_frame_png(image, "SCIENCE_COSMIC", ctx, channel, star=star)
    

    image = image * ccd_gain
    ctx.write_calibration_frame_png(image, "SCIENCE_COMPLETELY_MERGED", ctx, channel, star=star)
